...
export const Icon = ({ name, size = 14, ...rest }) => {
  const paths = {
    ...
    sort: <><path d="M3 6h14" /><path d="M3 12h10" /><path d="M3 18h6" /><path d="M19 4v16" /><path d="M15 16l4 4 4-4" /></>,
...
export function displayName(property) {
  return (property && (property.property_name || "").trim()) || "";
}

export function displayPropertyLabel(property) {
  return displayName(property) || displayAddress(property);
}

// ---------- Status badge ----------
export const LISTING_META = {
...
export function ListingBadge({ state }) {
  const m = LISTING_META[state] || LISTING_META.off_market;
  return <span className={`badge ${m.cls}`}>{m.label}</span>;
}

// ---------- Delta cell ----------
export function DeltaCell({ value, base }) {
...
// ---------- Range pill ----------
export function RangePill({ low, mid, high }) {
  return (
...
// ---------- Sortable Header ----------
export function SortHeader({ label, k, sort, setSort, align = "left", style, defaultDir = "asc" }) {
...
export function JsonViewer({ data, maxHeight }) {
...
// ---------- Sparkline ----------
export function Sparkline({ values, width = 60, height = 16, color }) {
...
export function bestEstimate(rawOrCurrent) {
...
//
// The two libraries are lazy-loaded (dynamic import → separate Vite chunk) the
// first time a Markdown component mounts — i.e. only when an AI answer is
// actually shown — so they are NOT fetched on page load, on pages without AI,
// or when AI is disabled. Until they arrive (and as a permanent fallback if
// the load fails) the raw text is shown pre-wrapped.
let _mdLibs = null;        // { marked, DOMPurify } once loaded
...
function ensureMarkdownLibs() {
  ...
  if (!_mdLibsPromise) {
    _mdLibsPromise = Promise.all([
      ...
      _mdLibs = { marked: markedMod.marked, DOMPurify: dompurifyMod.default };
      return _mdLibs;
...
export function Markdown({ text, className }) {
  const [ready, setReady] = useState(Boolean(_mdLibs));
  ...
  const html = useMemo(() => {
    ...
    if (!src.trim() || !_mdLibs) return null;
    const raw = _mdLibs.marked.parse(src, { breaks: true, gfm: true });
    return _mdLibs.DOMPurify.sanitize(raw, { USE_PROFILES: { html: true } });
  }, [text, ready]);
